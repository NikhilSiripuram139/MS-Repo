...
export class AppComponent implements OnInit {
  ...
  redirectUri = 'http://localhost:3000/account/auth/callback';
  post_logout_redirect_uri = 'http://localhost:3000/account/auth/logout';
  ...
  async ngOnInit() {
    try {
      await this.http.get("http://localhost:3000/user/all").subscribe((data) => {
        this.users = data;
      });
  ...
  async onlogin(user: any) {
    ...
    if (this.user === undefined || this.user.id !== user.id) {
      ...
      try {
        ...
        await this.http.post('http://localhost:3000/account/login', user).subscribe(async (res) => {
          if (res) {
            await this.http.get("http://localhost:3000/user/all").subscribe((data) => {
              this.users = data;
  ...
  async gotoWord(userId: string) {
    ...
    try {
      await this.http.post('http://localhost:3000/account/token', data).subscribe((data: any) => {
        if (data) {
          console.log(data);
  ...
  async gotoExcel(userId: string) {
    ...
    const data = { id: userId };
    try {
      await this.http.post('http://localhost:3000/account/token', data).subscribe((data) => {
        if (data) {
          console.log(data);
  ...
  async logout(userId: string) {
    try {
      ...
      const data = { userId }
      await this.http.post('http://localhost:3000/account/logout', data).subscribe(() => {
